...
def norm_data():
    ...
    for date in all_trade_date:
        ...
        ret[:, 0] = np.log1p(data[:, 1] / data[:, 0])
        ret[:, 1] = np.log1p(data[:, 2] / data[:, 0])
        ret[:, 2] = np.log1p(data[:, 3] / data[:, 0])
        ret[:, 3] = np.log1p(data[:, 0] / data[:, 3])
        ret[:, 4] = np.log1p(data[:, 1] / data[:, 3])
        ret[:, 5] = np.log1p(data[:, 2] / data[:, 3])
        ret[:, 6] = data[:, 4]
